--USE [Dabur_Central]
--GO
--/****** Object:  StoredProcedure [dbo].[DAB_ECO_BRAND_SKU_REPORT]    Script Date: 06/29/2023 3:27:37 PM ******/
--SET ANSI_NULLS ON
--GO
...
----USE [Dabur_Central]
----GO
----/****** Object:  StoredProcedure [dbo].[DAB_ECO_BRAND_SKU_REPORT]    Script Date: 06/28/2023 4:44:18 PM ******/
----SET ANSI_NULLS ON
----GO
----SET QUOTED_IDENTIFIER ON
----GO
...
SELECT @DATE2='2023-04-27' ,@REPMODE=1,@SUMMARY=3
--,@BRAND_CHECK   = 1,
--@SIZE_CHECK   = 0
IF(@REPORTTYPE <> 'CUS')
...
SELECT A.TRNDATE,A.PARAC,GRA.DRCP ,MI.MCODE MMCODE ,MI.DESCA,MI.MCAT,IIF(@SUMMARY=0 OR @BRAND_CHECK=1,BRAND_MANUAL,'') BRAND_MANUAL    ,IIF(@SUMMARY=0 OR @SIZE_CHECK=1,SIZE_MANUAL,'') SIZE_MANUAL  ,G.RouteName [BEAT_NAME],F.NAME SALESMAN,C.ACNAME STOCKIESTNAME ,A.COMPANYID STOCKIESTID,B.MCODE  INTO #MAIN   FROM
 ...
 LEFT JOIN 
		...
		LEFT JOIN 
		(SELECT DISTINCT SSMCODE, COUNT(DISTINCT ACID)DRCP FROM RMD_ACLIST WITH (NOLOCK) GROUP BY SSMCODE) GRA ON GRA.SSMCODE=isnull(F.SSMCODE,'~')
		WHERE  
...
MCAT,
 ...
 select A.STOCKIESTNAME ,B.DRCP,OUTLET_LY,OUTLET_LM,OUTLET_CM from(
	SELECT STOCKIESTNAME  ,STOCKIESTID ,COUNT( DISTINCT IIF( TRNDATE BETWEEN DATEADD(year, -1, @DATE1) AND DATEADD(year, -1, @DATE2) ,PARAC, NULL))  AS OUTLET_LY , 
		...
		(
	SELECT B.COMPANYID,COUNT(DISTINCT ACID) DRCP FROM RMD_ACLIST A WITH (NOLOCK)   JOIN SALESMAN B WITH (NOLOCK)   ON A.SSMCODE = B.SSMCODE GROUP BY B.COMPANYID
   )B
...
IF OBJECT_ID('tempdb..#DRCP') is not null drop table #DRCP

(SELECT COUNT(DISTINCT a.ACID) DRCP ,b.BusinessType INTO #DRCP FROM RMD_ACLIST a WITH (NOLOCK)join rmd_aclist b WITH (NOLOCK) on a.companyid= b.customerid WHERE ISNULL(a.SSMCODE,'') <>'' group by   b.BusinessType HAVING  b.BusinessType<>'COMMON')  
...
JOIN (SELECT DISTINCT MCODE,CATEGORYCODE,DESCA,MCAT,BRAND_MANUAL,SIZE_MANUAL FROM MENUITEM) MI ON MI.MCODE=B.MCODE 
 LEFT JOIN 
		...
		LEFT JOIN RMD_ACLIST C WITH (NOLOCK) ON A.COMPANYID =C.customerID
		LEFT JOIN 
		(SELECT DISTINCT SSMCODE, COUNT(DISTINCT ACID)DRCP FROM RMD_ACLIST WITH (NOLOCK) GROUP BY SSMCODE) GRA ON GRA.SSMCODE=F.SSMCODE
		WHERE 
		(@MCODE = '%' OR ( @MCODE <> '%' AND B.MCODE IN (SELECT * FROM DBO.SPLIT(@MCODE,',')))) 
		AND(@MCAT = '%' OR ( @MCAT <> '%' AND MI.MCAT IN (SELECT * FROM DBO.SPLIT(@MCAT,','))))   
		AND(@BRAND = '%' OR ( @BRAND <> '%' AND MI.BRAND_MANUAL  IN (SELECT * FROM DBO.SPLIT(@BRAND,','))))	
 ...
 SELECT    	EC.BRAND_MANUAL, EC.SIZE_MANUAL,	EC.MCAT,EC.SALESMAN,	EC.ECO_LY	,EC.ECO_LM	,EC.ECO_CM 	 , 
	 OLBR.OUTLET_LY ,
	 OLBR.OUTLET_LM  	OUTLET_LM,
 OLBR.OUTLET_CM 	OUTLET_CM 
...
LEFT JOIN
 ...
 SELECT MCAT, 
	...
	FROM #MAIN2  GROUP BY MCAT ) OLBR

	ON OLBR.MCAT=EC.MCAT
...
,	 DIVISIONS DIVISION,  DRCP,ECO_LY	,ECO_LM,	ECO_CM,	OUTLET_LY,	OUTLET_LM	,OUTLET_CM	,
 ...
 left join  
  (SELECT A.DRCP+B.DRCP DRCP,A.BusinessType  FROM  (
(SELECT * FROM #DRCP WHERE BusinessType<>'COMMON') A
  CROSS APPLY
(SELECT * FROM #DRCP WHERE BusinessType='COMMON')  B
 ) )DRCP ON DRCP.BUSINESSTYPE=RX.DIVISIONS
