...
--DECLARE
	@DATE1 DATETIME,
	@DATE2 DATETIME,
	@SALESMANID NVARCHAR(MAX)='%',--AGENT
	@CUSTOMERACID VARCHAR(MAX)='%',
...
--SELECT @DATE1='2022-05-04',@DATE2='2023-05-22'
	...
	BEGIN
		...
		AND	(@SALESMANID = '%' OR ( @SALESMANID <> '%' AND TM.SALESMANID  IN (SELECT * FROM DBO.SPLIT(@SALESMANID,',')))) 
		 AND	(@CUSTOMERACID = '%' OR ( @CUSTOMERACID <> '%' AND TM.TRNAC  IN (SELECT * FROM DBO.SPLIT(@CUSTOMERACID,','))))
	END
