...
AS
--select @CURRENTMONTH=2,@PHISCALYEAR='80/81',@CONFIGMODE='1'
...
@SALVAGEVALUE  SALVAGEVALUE,
@MinDepValue  MinDepValue,
@CALCULATION_INTV CALCULATION_INTV ,
@DEPFROM DEPFROM   , 
@ENABLELBD ENABLELBD ,  
...
WHERE AD.PhiscalID = @PHISCALYEAR
	AND  (	AD.BOOKVALUE - ISNULL(DP.DEPRECIATION_VALUE, 0))   > @MinDepValue AND
	AM.STATUS=1
...
SELECT AssetID,
	...
	AMI.MENUCODE,
	BATCHCODE,
  IIF(COSTPRICE<=@SALVAGEVALUE,DPBOOKVALUE,IIF(ABS(DPBOOKVALUE - DEPRECIATION_VALUE)<@SALVAGEVALUE,@SALVAGEVALUE,(DPBOOKVALUE - DEPRECIATION_VALUE)) )  BOOKVALUE,
	COSTPRICE ORIGNALCOST,
	  IIF(COSTPRICE<=@SALVAGEVALUE,0, IIF(ABS(DPBOOKVALUE - DEPRECIATION_VALUE)<@SALVAGEVALUE,DPBOOKVALUE-@SALVAGEVALUE,DEPRECIATION_VALUE))  DEPRECIATION,
	@CURRENTMONTH [MONTHS],
